...
fn runtime_diagnostics(app: tauri::AppHandle) -> Result<RuntimeDiagnostics, String> {
  ...
  let version_output = Command::new(&runtime).arg("version").output();
...
#[tauri::command]
fn run_k6(app: tauri::AppHandle, script: String, username: String, quiet: bool) -> Result<String, String> {
  let runtime = ensure_runtime(&app)?;
  let app_data = app
    .path()
    .app_data_dir()
