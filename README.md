...
- **k6** on your PATH for local `tauri dev` if you skip bundling; release installers should include k6 via `npm run k6:bundle-download` (see below). The web UI can fall back to browser `fetch` with CORS limits (see in-app diagnostics)  
...
### Shareable build for testers (bundles k6)

The desktop app expects a k6 binary under `src-tauri/resources/k6/` with fixed names (see `src-tauri/resources/k6/README.md`). Those files are gitignored so clones stay small; download them before you package:
...
Or in one step: `npm run tauri:build:full`.

Give your friend the **installer** from `src-tauri/target/release/bundle/` (for example the **NSIS** `.exe` under `nsis/` on Windows). They do **not** need Node, Rust, or a separate k6 install if that installer was built with the step above; they may need the **Microsoft Edge WebView2 Runtime** on some Windows machines if the window fails to open.

Workspace data (saved projects and collections) lives in the app’s user data folder after they run the app; it is not shipped inside the installer unless you add your own onboarding or sample import flow.
