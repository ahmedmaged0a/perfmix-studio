  ...
  return {
    ...
    apiRequests: dto.api_requests.map((item) => ({
      ...
      testCases: (item.test_cases ?? []).map((tc) => ({
        ...
        rampUp: tc.ramp_up,
        ...(typeof tc.ramp_down_enabled === 'boolean' ? { rampDownEnabled: tc.ramp_down_enabled } : {}),
        ...(typeof tc.ramp_down === 'string' ? { rampDown: tc.ramp_down } : {}),
  ...
  return {
    ...
    api_requests: data.apiRequests.map((item) => ({
      ...
      test_cases: item.testCases.map((tc) => ({
        ...
        duration: tc.duration,
        ramp_up: tc.rampUp,
        ...(typeof tc.rampDownEnabled === 'boolean' ? { ramp_down_enabled: tc.rampDownEnabled } : {}),
        ...(typeof tc.rampDown === 'string' ? { ramp_down: tc.rampDown } : {}),
