...
import type { Collection, CorrelationRule, RequestDefinition } from '../../models/types'
...
import { parseJmx, type JmxParseOptions } from '../jmxParse'
...
type ImportCollectionModalProps = {
  open: boolean
  onClose: () => void
  onConfirm: (collection: Collection, correlationRules?: CorrelationRule[]) => void
}
...
export function ImportCollectionModal(props: ImportCollectionModalProps) {
  ...
  const submit = () => {
    ...
    props.onConfirm(res.collection, res.correlationRules)
    setText('')
    props.onClose()
  }
  ...
  return (
    <ModalBackdrop onClose={props.onClose}>
      <div className="ws-modal" role="dialog" aria-labelledby="ws-import-col-title">
        ...
        <div className="ws-modal-body">
          <p className="muted tiny">Paste a PerfMix collection JSON export (perfMixCollectionExport v1 or v2).</p>
...
export function ImportJmxModal(props: ImportJmxModalProps) {
  ...
  const [correlationDebug, setCorrelationDebug] = useState(false)
  ...
  useEffect(() => {
    if (!props.open) {
      ...
      setError(null)
      setPreview(null)
      setCorrelationDebug(false)
    }
  }, [props.open])

  const jmxOpts = useMemo((): JmxParseOptions => ({ correlationDebug }), [correlationDebug])
  ...
  const handleFileText = (text: string) => {
    ...
    const res = parseJmx(text, jmxOpts)
    if (res.ok) {
      setPreview({
        collectionName: res.collection.name,
        requestCount: res.collection.requests.length,
        variableCount: Object.keys(res.collection.variables ?? {}).length,
  ...
  useEffect(() => {
    if (!props.open || !xmlText.trim()) return
    const res = parseJmx(xmlText, jmxOpts)
    if (res.ok) {
      setPreview({
        collectionName: res.collection.name,
        requestCount: res.collection.requests.length,
  ...
  }, [correlationDebug, jmxOpts, props.open, xmlText])
        ...
        <div className="ws-modal-body">
          ...
          <div className="ws-import-section">
            <p className="ws-import-section-desc muted">
              Select a JMeter <code>.jmx</code> file to convert HTTP Request samplers (<code>HTTPSamplerProxy</code> and
              legacy <code>HTTPSampler</code>) into a PerfMix collection. JDBC, JMS, JSR223Sampler, and other non-HTTP
              samplers are skipped (see import warnings). Headers, body, query parameters, and protocol are preserved.
            </p>
          </div>
          ...
          <label className="ws-inline" style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              type="checkbox"
              checked={correlationDebug}
              onChange={(e) => setCorrelationDebug(e.target.checked)}
            />
            <span className="muted tiny">
              Emit correlation debug in k6 (console.warn when RUNTIME extractors are empty after each request)
            </span>
          </label>
            ...
            <div className="ws-import-result" style={{ marginTop: 12 }}>
              ...
              {preview.variableCount > 0 ? (
                <p className="muted tiny" style={{ marginTop: 8 }}>
                  {preview.variableCount} user-defined variable{preview.variableCount !== 1 ? 's' : ''} → collection
                  variables; JMeter <code className="mono">{'${variable}'}</code> →{' '}
                  <code className="mono">{'{{variable}}'}</code>
                </p>
              ) : null}
              <p className="muted tiny" style={{ marginTop: preview.variableCount > 0 ? 4 : 8 }}>
                Thread Group load (VUs, ramp, duration) is mapped to one test case per request where possible.
              </p>
          ...
          <details className="ws-import-notes" style={{ marginTop: 12 }}>
            ...
            <ul className="muted tiny ws-import-notes-list">
              ...
              <li>User-defined variables and Thread Group load settings → collection variables and per-request test cases</li>
              <li>Controllers (If, Loop, Transaction) are flattened — requests are extracted</li>
              <li>Regex extractors → Extract rules; JSR223 PostProcessor scripts → request “JSR223 (JMeter)” tab + props/vars.put shim after Send extractors; JSON PostProcessor not imported yet</li>
            </ul>
          </details>
        </div>
        <div className="ws-modal-foot">
...
type ImportUnifiedModalProps = {
  ...
  onConfirmCollection: (collection: Collection, correlationRules?: CorrelationRule[]) => void
  onConfirmJmx: (collection: Collection, correlationRules: CorrelationRule[]) => void
...
function ImportCollectionBody(props: {
  ...
  const submit = () => {
    if (!result?.ok) return
    props.onConfirm(result.collection, result.correlationRules)
    props.onClose()
  }
...
function ImportJmxBody(props: { onClose: () => void; onConfirm: (collection: Collection, correlationRules: CorrelationRule[]) => void }) {
  const [rawJmx, setRawJmx] = useState<string | null>(null)
  const [correlationDebug, setCorrelationDebug] = useState(false)
  const jmxOpts = useMemo((): JmxParseOptions => ({ correlationDebug }), [correlationDebug])
  const result = useMemo(() => (rawJmx ? parseJmx(rawJmx, jmxOpts) : null), [rawJmx, jmxOpts])
  const submit = () => {
    if (!result?.ok) return
    props.onConfirm(result.collection, result.correlationRules)
    props.onClose()
  ...
  return (
    <>
      <FileDropZone accept=".jmx" label="Drop a JMeter .jmx file here" hint=".jmx — or click to browse" onFileText={(text) => setRawJmx(text)} />
      <label className="ws-inline" style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
        <input type="checkbox" checked={correlationDebug} onChange={(e) => setCorrelationDebug(e.target.checked)} />
        <span className="muted tiny">Emit correlation debug in k6 export</span>
      </label>
