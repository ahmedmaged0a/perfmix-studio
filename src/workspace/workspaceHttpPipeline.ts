...
export type HttpPipelineOptions = {
  ...
  /** Same batch as templateCtx.httpCookieSessionId; forwarded to executeHttpRequest. */
...
export type HttpPipelineResult = {
  ...
  /** Snapshot of headers/body sent (or attempted) on the wire. */
...
function buildOutgoingRequestSnapshot(resolved: {
  method: string
  headers: Record<string, string>
  body: string
}): { requestHeaders: [string, string][]; requestBody: string } {
  const requestHeaders = Object.keys(resolved.headers)
    .sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase()))
    .map((k) => [k, String(resolved.headers[k] ?? '')] as [string, string])
  const methodUp = resolved.method.toUpperCase()
  const requestBody =
    methodUp === 'GET' || methodUp === 'HEAD' ? '' : String(resolved.body ?? '')
...
function basePm(
  ...
  const pm = {
    ...
    sendRequest(
      ...
      void (async () => {
        try {
          const sid = cookieSessionId ?? ctx.httpCookieSessionId
...
function createPrePm(
  ...
  return basePm(ctx, draft, scriptLogs, cookieSessionId)
}
...
function createPostPm(
  ...
  const { pm, console } = basePm(ctx, draft, scriptLogs, cookieSessionId)
...
export async function runSingleRequestHttpPipeline(opts: HttpPipelineOptions): Promise<HttpPipelineResult> {
  ...
  const cookieSessionId = opts.cookieSessionId ?? opts.templateCtx.httpCookieSessionId
  ...
  if (pre?.trim()) {
    try {
      const { pm, console } = createPrePm(templateCtx, draft, scriptLogs, cookieSessionId)
      ...
      const msg = e instanceof Error ? e.message : String(e)
      const snap = buildOutgoingRequestSnapshot({
        method: draft.method,
  ...
  const outgoing = buildOutgoingRequestSnapshot(resolved)
  ...
  if (post?.trim()) {
    try {
      const { pm, console } = createPostPm(templateCtx, draft, result, scriptLogs, cookieSessionId)
      ...
      return {
        ...
        requestBody: outgoing.requestBody,
