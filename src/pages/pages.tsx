...
import { buildCodegenRunLabelStem } from '../workspace/runLabel'
...
export function CodegenPage() {
  ...
  const runNow = async () => {
    ...
    await executeK6Run(
      buildCodegenRunLabelStem({
        mode,
        scenarioName,
        requestDisplayName: mode === 'single' ? req?.name : undefined,
      }),
    )
...
export function ReportsPage() {
  ...
  const downloadHtml = async () => {
    ...
    const result = await exportHtmlFile({ html, defaultFileName: `${lastRunId}.report.html` })
    if (!result.ok) {
