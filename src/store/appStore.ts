...
import { DEFAULT_K6_LOAD_FIELDS, ensureCollectionK6LoadFields } from '../workspace/k6LoadDefaults'
import { isRunnableK6Script } from '../workspace/k6ScriptRunnable'
...
function migrateLegacyRequestsToProject(data: AppData): AppData {
  ...
  for (const [name, reqs] of folders.entries()) {
    collections.push({ id: buildId('col'), name, requests: reqs, variables: {}, ...DEFAULT_K6_LOAD_FIELDS })
  }
...
function storageKeyForUser(): string | null {
  const email = useAuthStore.getState().email?.trim()
  ...
  return {
    ...
    projects: (migrated.projects ?? []).map((p) => ({
      ...
      collections: (p.collections ?? []).map((c) => ({
        ...c,
        ...ensureCollectionK6LoadFields(c),
...
type AppStore = {
  ...
  ) => Promise<{ runId: string | null; status: K6RunStatus | 'idle' | 'failed'; summaryJson: string | null }>
...
function persistData(data: AppData) {
  saveData(data)
  const userEmail = useAuthStore.getState().email?.trim() ?? ''
  void apiClient.saveAppData(userEmail, data)
}
...
export const useAppStore = create<AppStore>((set, get) => ({
  ...
  loadData: async () => {
    ...
    try {
      ...
      const userEmail = useAuthStore.getState().email?.trim() ?? ''
      const remote = await apiClient.getAppData(userEmail)
  ...
  executeK6Run: async (runLabelStem?: string | null) => {
    const script = get().generatedScript
    if (!isRunnableK6Script(script)) {
      const msg =
    ...
    try {
      ...
      const start = await apiClient.startK6Run(script, userEmail, quiet, runLabelStem)
      ...
      set({
        error: null,
        lastRunStatus: 'failed',
        runLogs: [...get().runLogs, `Failed to execute k6 run: ${detail}`],
      })
      return { runId: activeRunId, status: 'failed', summaryJson: get().lastSummaryJson }
    }
  },
