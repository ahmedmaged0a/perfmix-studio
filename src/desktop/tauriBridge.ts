...
export async function tauriRunK6(script: string, username: string, quiet: boolean): Promise<{ runId: string } | null> {
  if (!hasTauriRuntime()) return null
  const runId = await invoke<string>('run_k6', { script, username, quiet })
  return { runId }
}
